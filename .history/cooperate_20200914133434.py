...
def popPeo(p,v,valCalculated=True):
	for id in range(p):
		peo[id]={
			#1 - people's value
			'value':np.array([sts.lognorm.rvs(.5)*v]) if valCalculated else v,
			#2 - people's ability
			'ability':np.array([(1/(sts.lognorm.rvs(.99)*100))]),
			#3 - help needed
			'helpNeeded':np.array([(1/(sts.lognorm.rvs(.99)+1))*100]),
			#4 - people helped
			'helpOut':np.zeros(numOfM),
			#5 - people who helped you
			'helpIn':np.zeros(numOfM)
		}
	return peo
invest=popPeo(numOfInvest,2000000)
oper=popPeo(numOfOper,75000)
assoc=popPeo(numOfAssoc,2500,False)
part=popPeo(numOfPart,7500)
totalValue=calTotalVal(invest)+calTotalVal(oper)+calTotalVal(assoc)+calTotalVal(part)
totalValue
bufferFund=0
